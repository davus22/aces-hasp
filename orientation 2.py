...
df = pd.read_csv('actual_data.csv',usecols=['accel-x', 'accel-y', 'accel-z','mag-x', 'mag-y', 'mag-z', 'gyro-x', 'gyro-y', 'gyro-z','lat','long','seconds'])
#print(df)
...
acc_1 = np.cos(yaw)*np.cos(pitch) 
acc_2 = np.cos(yaw)*np.sin(pitch)*np.sin(roll)-np.sin(yaw)*np.cos(roll)
acc_3 = np.cos(yaw)*np.sin(pitch)*np.cos(roll)+np.sin(yaw)*np.sin(roll)
acc_4 = np.sin(yaw)*np.cos(pitch)
acc_5 = np.sin(yaw)*np.sin(pitch)*np.sin(roll)+np.cos(yaw)*np.cos(roll)
acc_6 = np.sin(yaw)*np.sin(pitch)*np.cos(roll)-np.cos(yaw)*np.sin(roll)
acc_7 = -np.sin(pitch)
